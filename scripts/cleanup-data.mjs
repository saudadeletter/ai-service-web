...
try {
  ...
  const { rows } = await client.query(
    'SELECT count(*)::int AS count FROM "ServiceRequest" WHERE "status" IN (\'COMPLETED\', \'CLOSED\') AND "updatedAt" < $1',
    [cutoff],
  );
  console.log(
    `符合清理条件的需求：${rows[0].count} 条（完成/关闭且超过 90 天未更新）。`,
  );
  if (!apply) {
    ...
    await client.query(
      'DELETE FROM "ServiceRequest" WHERE "status" IN (\'COMPLETED\', \'CLOSED\') AND "updatedAt" < $1',
      [cutoff],
    );
